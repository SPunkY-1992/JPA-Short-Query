...
public interface GreaterThan {
  ...
  Specification<S> greaterThan(J1 join1, SingularAttribute<TJ1, A> attribute, A value) {
    return (root, cq, cb) ->
        value == null
            ? cb.disjunction()
            : cb.greaterThan(RawJoiner.<S, TJ1>join(root, join1).get(attribute), value);
  }

  /**
   * example of usage: greaterThan(User_.projects, Project_.subProjects, SubProject_.number, 666)
   */
  default <S extends T, T,
  ...
  Specification<S> greaterThan(J1 join1, J2 join2, SingularAttribute<TJ2, A> attribute, A value) {
    return (root, cq, cb) ->
        value == null
            ? cb.disjunction()
            : cb.greaterThan(RawJoiner.<S, TJ2>join(root, join1, join2).get(attribute), value);
  }
  ...
  Specification<S> greaterThan(J1 j1, J2 j2, J3 j3, SingularAttribute<TJ3, A> attribute, A value) {
    return (root, cq, cb) ->
        value == null
            ? cb.disjunction()
            : cb.greaterThan(RawJoiner.<S, TJ3>join(root, j1, j2, j3).get(attribute), value);
  }
  ...
  Specification<S> greaterThan(J1 j1, J2 j2, J3 j3, J4 j4, SingularAttribute<TJ4, A> a, A value) {
    return (root, cq, cb) ->
        value == null
            ? cb.disjunction()
            : cb.greaterThan(RawJoiner.<S, TJ4>join(root, j1, j2, j3, j4).get(a), value);
  }
