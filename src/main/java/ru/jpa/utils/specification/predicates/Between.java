...
public interface Between {
  ...
  Predicate between(Path<A> path, A lower, A upper, CriteriaBuilder cb) {
    if (lower == null || upper == null) {
      return cb.disjunction();
    }
    if (lower instanceof LocalDateTime) {
      return cb.between(
          cb.function("TO_TIMESTAMP", String.class, path, cb.literal("YYYY-MM-DD HH24:MI:SS.FF 6")),
          dateTimeWithMicroseconds((LocalDateTime) lower),
          dateTimeWithMicroseconds((LocalDateTime) upper)
      );
    }
    if (lower instanceof ZonedDateTime) {
      return cb.between(
          cb.function("TO_TIMESTAMP", String.class, path, cb.literal("YYYY-MM-DD HH24:MI:SS.FF 6")),
          dateTimeWithMicroseconds((ZonedDateTime) lower),
          dateTimeWithMicroseconds((ZonedDateTime) upper)
      );
    }
  ...
  Specification<S> between(SingularAttribute<T, A> attribute, A lower, A upper) {
    return (root, cq, cb) -> between(root.get(attribute), lower, upper, cb);
  }
  ...
  Specification<S> between(J1 join1, SingularAttribute<TJ1, A> attribute, A lower, A upper) {
    return distinct((root, cq, cb) -> between(join(root, join1).get(attribute), lower, upper, cb));
  }
  ...
  Specification<S> between(J1 join1, J2 join2, SingularAttribute<TJ2, A> attribute, A l, A u) {
    return distinct((root, cq, cb) -> between(join(root, join1, join2).get(attribute), l, u, cb));
  }
  ...
  Specification<S> between(J1 j1, J2 j2, J3 j3, SingularAttribute<TJ3, A> attribute, A l, A u) {
    return distinct((root, cq, cb) -> between(join(root, j1, j2, j3).get(attribute), l, u, cb));
  }
  ...
  Specification<S> between(J1 j1, J2 j2, J3 j3, J4 j4, SingularAttribute<TJ4, A> a, A l, A u) {
    return distinct((root, cq, cb) -> between(join(root, j1, j2, j3, j4).get(a), l, u, cb));
  }
