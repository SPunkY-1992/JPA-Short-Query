...
public interface Like {
  ...
  /**
   * example of usage: like(User_.projects, Project_.subProjects, SubProject_.name, partOfName)
   */
  default <S extends T, T,
  ...
  Specification<S> like(J1 j1, J2 j2, J3 j3, J4 j4, SingularAttribute<TJ4, A> a, String value) {
    return (root, cq, cb) ->
        value == null
            ...
            : cb.like(
                cb.lower(
                    RawJoiner.<S, TJ4>join(root, j1, j2, j3, j4).get(a.getName()).as(String.class)),
                "%" + value.toLowerCase() + "%"
            );
  }
